...
def _gen_mm_extra_hash_keys(
    ...
    """Return multi-modal hash keys; this build is text-only, so none.
    ...
    Returns:
        An empty key list and the unchanged multi-modal index.
    """
    del request, start_token_idx, end_token_idx
    return [], start_mm_idx


def _gen_lora_extra_hash_keys(request: Request) -> list[str]:
    """Return LoRA hash keys; this build has no LoRA, so always empty."""
    del request
    return []
